...
function syncAdministrasiNow(options) {
  ...
  const records = values.map(function(v, i) {
    ...
    return { sourceKey: stable, activityId: activityId, reportId: 'LAP-' + activityId, company: company, category: category, region: adminText_(v[1]), kind: adminText_(v[10]) || adminText_(v[9]) || 'Administrasi', location: adminText_(v[31]), pic: adminText_(v[35]) || adminText_(v[37]), year: Number(String(incoming || start || end || Utilities.formatDate(new Date(), APP.TIMEZONE, 'yyyy')).slice(0, 4)), incoming: incoming, start: start, end: end, sent: sent, value: Number(v[30]) || Number(v[28]) || 0, invoice: adminText_(v[42]) || adminText_(v[22]), invoiceDate: sourceDate_(v[43]), due: sourceDate_(v[45]), noIncoming: adminText_(v[7]), journal: adminText_(v[46]) || adminText_(v[40]), status: adminText_(v[4]) || 'PROSES', note: ['SOURCE_SYNC=ADM', adminText_(v[9]), adminText_(v[24]), adminText_(v[47])].filter(Boolean).join(' | ') };
  }).filter(Boolean);
  ...
  const tx = withWriteTransaction_({actor:currentUser_(), action:'sync_adm_source', tableName:'MULTI', recordId:sourceId, reason:'Sinkronisasi Google Sheet Administrasi'}, function(master) {
    ...
    records.forEach(function(r){
      ...
      const ar=upsertMappedRecord_(as,'activity_id',r.activityId,{activity_id:r.activityId,display_id:r.sourceKey,company_id:cid,perusahaan:r.company,subbagian:r.category,kategori:r.category==='RPJID'?'RP':r.category==='PLT'?'TR':r.category,instansi:r.region,jenis_kegiatan:r.kind,status_biaya:r.status,regional:r.region,kebun_lokasi:r.location,tahun:r.year,tanggal_surat_masuk:r.incoming,no_surat_masuk:r.noIncoming,tanggal_mulai:r.start,tanggal_selesai:r.end,nilai_kontrak:r.value,pic:r.pic,status:r.sent?'SELESAI':'AKTIF',catatan:r.note,created_at:nowIso_(),updated_at:nowIso_(),archived_at:''}); if(ar==='inserted')ia++;else ua++;
      const rr=upsertMappedRecord_(rs,'report_id',r.reportId,{report_id:r.reportId,activity_id:r.activityId,company_id:cid,perusahaan:r.company,regional:r.region,kebun:r.location,nama_kegiatan:r.kind,tahun:r.year,workflow:r.category==='RPJID'?'RP':(r.category==='ADM'?'ADMINISTRASI':'UMUM'),tanggal_draft_masuk:r.incoming,checkpoint_terakhir:r.sent?'SELESAI':(r.end?'PELAKSANAAN':(r.incoming?'DRAFT':'')),tanggal_checkpoint:r.sent||r.end||r.incoming,tanggal_kirim:r.sent,status:r.sent?'NET':r.status,pic:r.pic,catatan:r.note,created_at:nowIso_(),updated_at:nowIso_(),archived_at:''}); if(rr==='inserted')ir++;else ur++;
      if(r.value||r.invoice||r.invoiceDate){const br=upsertMappedRecord_(bs,'billing_id','BILL-'+r.activityId,{billing_id:'BILL-'+r.activityId,company_id:cid,perusahaan:r.company,kebun:r.location,source_type:r.category==='ADM'?'ADMINISTRASI':'KEGIATAN',source_id:r.activityId,nilai:r.value,nomor_invoice:r.invoice,tanggal_invoice:r.invoiceDate,jatuh_tempo:r.due,status:r.invoice?'TERBIT':'DRAFT',nomor_jurnal:r.journal,pic:r.pic,catatan:r.note,created_at:nowIso_(),updated_at:nowIso_(),archived_at:''});if(br==='inserted')ib++;else ub++;}
    });
    const activityIndex={}, sourceIndex={}; records.forEach(function(r){activityIndex[r.company.toLowerCase()+'|'+r.kind.toLowerCase()]=r;sourceIndex[r.sourceKey]=r;});
    reportRows.forEach(function(v,i){
      const sourceRecordId=adminText_(v[0]), company=adminText_(v[1]), kind=adminText_(v[3])||adminText_(v[2]); if(!company) return;
      const linked=sourceIndex[sourceRecordId]||activityIndex[company.toLowerCase()+'|'+kind.toLowerCase()]||records.find(function(r){return r.company.toLowerCase()===company.toLowerCase();}); if(!linked) return;
      const rid=linked.reportId, draftDate=sourceDate_(v[6]), reportYear=Number(String(draftDate||linked.incoming||linked.start||Utilities.formatDate(new Date(), APP.TIMEZONE, 'yyyy')).slice(0,4));
      const rr=upsertMappedRecord_(rs,'report_id',rid,{report_id:rid,activity_id:linked.activityId,company_id:companies[company.toLowerCase()]||'',perusahaan:company,nama_kegiatan:kind,tahun:reportYear,workflow:linked.category==='RPJID'?'RP':(linked.category==='ADM'?'ADMINISTRASI':'UMUM'),tanggal_draft_masuk:draftDate,tanggal_checkpoint:sourceDate_(v[30])||sourceDate_(v[29])||sourceDate_(v[21]),tanggal_cetak:sourceDate_(v[29]),tanggal_kirim:sourceDate_(v[30]),checkpoint_terakhir:sourceDate_(v[30])?'SELESAI':(sourceDate_(v[29])?'CETAK NET':(sourceDate_(v[21])?'CETAK 1':'DRAFT')),status:sourceDate_(v[30])?'NET':'PROSES',pic:adminText_(v[7])||linked.pic,catatan:'SOURCE_SYNC=ADM/LAPORAN | SOURCE_ID='+sourceRecordId,created_at:nowIso_(),updated_at:nowIso_(),archived_at:''}); if(rr==='inserted')ir++;else ur++;
    ...
    teamRows.forEach(function(v,i){
      const linked=sourceIndex[adminText_(v[0])]||records[i]; if(!linked) return;
