...
function syncAdministrasiNow(options) {
  ...
  let ia=0,ua=0,ir=0,ur=0,ib=0,ub=0,cc=0;
  ...
  return success_({sourceSpreadsheetId:sourceId,sourceUrl:source.getUrl(),rowsRead:records.length,insertedActivities:ia,updatedActivities:ua,insertedReports:ir,updatedReports:ur,insertedBilling:ib,updatedBilling:ub,companiesCreated:cc,backupId:tx.backupId,message:records.length+' baris Administrasi disinkronkan.'});
}
